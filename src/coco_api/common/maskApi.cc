...
void rleInit( RLE *R, siz h, siz w, siz m, uint *cnts ) {
  R->h=h; R->w=w; R->m=m; R->cnts=(m==0)?0:static_cast<uint*>(malloc(sizeof(uint)*m));
...
void rlesInit( RLE **R, siz n ) {
  siz i; *R = (RLE*) static_cast<RLE*>(malloc(sizeof(RLE)*n));
...
void rleEncode( RLE *R, const byte *M, siz h, siz w, siz n ) {
  ...
  cnts = static_cast<uint*>(malloc(sizeof(uint)*(a+1)));
  for(i=0; i<n; i++) {
...
void rleMerge( const RLE *R, RLE *M, siz n, int intersect ) {
  ...
  cnts = static_cast<uint*>(malloc(sizeof(uint)*(h*w+1)));
...
void rleIou( RLE *dt, RLE *gt, siz m, siz n, byte *iscrowd, double *o ) {
  ...
  db=static_cast<double*>(malloc(sizeof(double)*m*4)); rleToBbox(dt,db,m);
  gb=static_cast<double*>(malloc(sizeof(double)*n*4)); rleToBbox(gt,gb,n);
...
void rleFrPoly( RLE *R, const double *xy, siz k, siz h, siz w ) {
  ...
  x=static_cast<int*>(malloc(sizeof(int)*(k+1))); y=static_cast<int*>(malloc(sizeof(int)*(k+1)));
  for(j=0; j<k; j++) x[j]=(int)(scale*xy[j*2+0]+.5); x[k]=x[0];
  for(j=0; j<k; j++) y[j]=(int)(scale*xy[j*2+1]+.5); y[k]=y[0];
  for(j=0; j<k; j++) m+=umax(abs(x[j]-x[j+1]),abs(y[j]-y[j+1]))+1;
  u=static_cast<int*>(malloc(sizeof(int)*m)); v=static_cast<int*>(malloc(sizeof(int)*m)); m=0;
  for( j=0; j<k; j++ ) {
  ...
  x=static_cast<int*>(malloc(sizeof(int)*k)); y=static_cast<int*>(malloc(sizeof(int)*k));
  ...
  k=m; a=static_cast<uint*>(malloc(sizeof(uint)*(k+1)));
  ...
  b=static_cast<uint*>(malloc(sizeof(uint)*k)); j=m=0; b[m++]=a[j++];
...
char* rleToString( const RLE *R ) {
  ...
  char *s=static_cast<char*>(malloc(sizeof(char)*m*6));
  for( i=0; i<m; i++ ) {
...
void rleFrString( RLE *R, char *s, siz h, siz w ) {
  ...
  while( s[m] ) m++; cnts=static_cast<uint*>(malloc(sizeof(uint)*m)); m=0;
  while( s[p] ) {
    x=0; k=0; more=1;
    while( more ) {
